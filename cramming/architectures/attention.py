...
def get_attention_mechanism(
    ...
    elif cfg_attention.type == "topoformer":
        mechanism = TopoAttention(hidden_size, cfg_attention)
...
class TopoAttention(torch.nn.Module):
    """
    Minimal wrapper around topoformer attention. This module will use the AttentionOutput implemented in the topoformer library. 
    """
    ...
    def __init__(self, hidden_size, cfg_attention):
        ...
        self.output_dim = hidden_size
        self.output_projection = LocallyConnected(hidden_size, hidden_size, cfg_attention.sr, mask_type=cfg_attention.mask_type)
        # hidden size = attention head
        self.head = AttentionHead(
            hidden_size,
            hidden_size,
            cfg_attention.dropout_prob,
            cfg_attention.sq,
            cfg_attention.qkv_bias,
            cfg_attention.learned_spatial_querying,
            cfg_attention.local_querying,
            cfg_attention.mask_type,
            relu=cfg_attention.relu,
            transpose=True,
        )
      
    def forward(self, x, mask=None):
        assert mask is None, 'Topoformer does not yet support attention masks'
...
class SeqFirstSelfAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size: int, cfg_attention):
        super(SeqFirstSelfAttention, self).__init__()
        ...
        logging.info("Training without locality. Single Head Attention. This will be the baseline!!!")
    
    def forward(self, embeddings, mask):
        # Transpose the input to (batch, seq, hidden)
        embeddings = embeddings.transpose(0, 1)

        values = self.values(embeddings)  # (batch, seq_len, embed_size)
        keys = self.keys(embeddings)  # (batch, seq_len, embed_size)
        queries = self.queries(embeddings)  # (batch, seq_len, embed_size)
        ...
        out = torch.bmm(attention, values)  # (batch, seq_len, embed_size)
        out = self.fc_out(out)

        # Reshape back to original input shape: (seq, batch, hidden)
        out = out.transpose(0, 1)

        return out

class SeqFirstSelfAttentionn(torch.nn.Module):
    ...
    def __init__(self, hidden_size: int, cfg_attention):
        ...
        logging.info(f"number of attention heads: {cfg_attention.num_attention_heads}")
        ...
        self.fc_out = torch.nn.Linear(self.hidden_size, self.hidden_size)
        self.output_dim = hidden_size
    ...
    def forward(self, hidden_states, attention_mask: Optional[torch.Tensor] = None):
        ...
        context_layer = self.fc_out(context_layer)
        return context_layer
